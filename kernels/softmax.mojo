...
def reduce[
    ...
    while current_N > 1:
        ...
        if num_blocks == 1:
            ...
            var d_intermediate_buffer = ctx.enqueue_create_buffer[dtype](
                num_blocks
            )
...
def main():
    ...
    with DeviceContext() as ctx:
        ...
        reduce[max_op](
            ctx, input_tensor.ptr, max_buf.unsafe_ptr(), N, -inf[dtype]()
        )

        var elementwise_grid_size = ceildiv(N, BLOCK_SIZE)
        var block_sums_buf = ctx.enqueue_create_buffer[dtype](
            elementwise_grid_size
        )
